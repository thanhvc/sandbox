...
public class Activity<C> {
  ...
  private Activity(String caption, boolean isEnd) {
    this.caption = caption;
    this.transitions = new LinkedList<Transition<C>>();
    this.isFinal = isEnd;
  }
  
  public static Activity newInstance(String caption, boolean isFinal) {
    return new Activity(caption, isFinal);
  }
  
  public void connectTo(C letter, Activity<C> target) {
  ...
  public Activity<C> findTargetState(C letter) {
    for (Transition tran : transitions) {
